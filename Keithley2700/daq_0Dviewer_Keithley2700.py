...
class DAQ_0DViewer_Keithley2110(DAQ_Viewer_base):
    ...
    params = comon_parameters+[
        {'title': 'Keithley2210 Parameters',  'name': 'K2110Params', 'type': 'group', 'children': [
            {'title': 'Mode', 'name': 'mode', 'type': 'list', 'limits': ['VDC', 'VAC', 'R2W', 'R4W', 'TEMP'], 'value': 'VDC'}
    ...
    def ini_detector(self, controller=None):
        ...
        if self.settings.child(('controller_status')).value() == "Slave":
            ...
            try:
                ...
                self.controller = Keithley2110('ASRL1::INSTR')
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        data = self.controller.readvalue()
        #self.data_grabed_signal.emit([utils.DataFromPlugins(name='K2110', data=[[data]], dim='Data0D',)])
        self.data_grabed_signal.emit([DataFromPlugins(name='ASRL1::INSTR', data=[np.array([data])], dim='Data0D',)])
        self.ind_data += 1
